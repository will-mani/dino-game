...
class dino:
    ...
    def is_defenitely_on_ground(self):
        ...
        return y_position_delta == 0 and secs_on_ground > 2
    
    
    def update_obstacle_view_properties(self, game_thresh):
        if self.is_defenitely_on_ground():
            self.obstacle_view_min_x = self.left_most_point
    ...
    def jump_now(self, nearest_obstacle):
        if (not self.is_ascending_to_min_y and not self.is_over_min_y
            and self.right_most_point < nearest_obstacle.start_x and self.top_point < nearest_obstacle.bottom):

            adjusted_obstacle_velocity = nearest_obstacle.pixels_per_sec * 1.5
...
class obstacle:
    ...
    def update_pixels_per_sec(self, dino_right_most_point, dino_width):
...
while True:
    ...
    if game_dino_contour is not None:
        ...
        nearest_obstacle.update_pixels_per_sec(rex.right_most_point, rex.width)
