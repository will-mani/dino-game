...
def update_obstacle_list(dino: Dino, obstacle_list):
    view = dino.obstacle_view_thresh.copy()
    new_obstacle_list = []
    for i in range(len(obstacle_list)):
        obstacle = obstacle_list[i]
        obstacle.update_position_properties(view, dino.width)
    ...
    while True:
        ...
        new_obstacle.update_position_properties(view, dino.width)
...
async def press_down(approx_relase_time):
...
while True:
    ...
    if game_dino_contour is not None:
        ...
        obstacle_list = update_obstacle_list(rex, obstacle_list)
        ...
        if threatening_obstacle != None:
            if threatening_obstacle.bottom == rex.obstacle_view_max_y:
            ...
            elif rex.top_point < threatening_obstacle.bottom:
                ...
                if threatening_obstacle.pixels_per_sec > 0:
                ...
                asyncio.run(press_down(approx_relase_time))
